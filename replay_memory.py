...
class ReplayMemory:
    def __init__(self, model_dir):
        ...
        self.prestate = np.empty((self.memory_size, 66), dtype=np.float16)
        self.poststate = np.empty((self.memory_size, 66), dtype=np.float16)
        k = 1 + len(self.actions) / self.memory_size
        self.batch_size = int(k * 2000)
        self.count = 0
        self.current = 0
